# Dehumidifier Monitoring 101

## Related Content

🎙️ Tug's Garage Radio Episode — Coming Soon

🎥 Learning 101 Video — Coming Soon

## Why Monitor a Dehumidifier?

A dehumidifier is one of those appliances that quietly protects a home every day.

Most homeowners don't think about it until the basement starts feeling damp, humidity rises, or mold begins to appear.

The problem is that a dehumidifier can stop working without anyone noticing.

The bucket can fill up.

The unit can lose power.

A component can fail.

The machine can continue running but no longer remove moisture effectively.

The purpose of dehumidifier monitoring is awareness before moisture problems become expensive repairs.

What We Want To Know

- Is the dehumidifier running?
- Has the bucket filled up?
- Has power been lost?
- Is humidity increasing?
- Is the unit running normally?
- Is performance changing over time?

Monitoring Methods

Power Monitoring

Power monitoring allows us to determine when the unit is running and identify changes in operating behavior.

Humidity Monitoring

Humidity trends help us verify whether the dehumidifier is actually removing moisture from the space.

Runtime Tracking

Tracking runtime helps identify unusual operating patterns and potential performance issues.

Failure Modes

Bucket Full

The dehumidifier shuts off and humidity begins to rise.

Power Loss

The unit stops operating completely.

Mechanical Failure

The unit may run differently or stop operating normally.

Reduced Performance

Humidity remains elevated despite normal operation.

Tug's Garage Philosophy

This project follows the Tug's Garage principle of:

Awareness Before Automation.

The goal is not necessarily to control the dehumidifier.

The goal is to understand what the system is doing and receive early warning when something changes.

Monitoring creates awareness.

Awareness prevents surprises.

Future Improvements

- Runtime analytics
- Humidity trend analysis
- Predictive maintenance alerts
- Seasonal performance tracking
- Basement moisture dashboards
